class contrail::provision_metadata_services (
    ...
    exec { 'provision-metadata-services' :
            command   => "python /opt/contrail/utils/provision_linklocal.py --api_server_ip \"${api_server_ip}\" --api_server_port \"${api_server_port}\" --admin_user \"${keystone_admin_user}\" --admin_password \"${keystone_admin_password}\" --linklocal_service_name metadata --linklocal_service_ip 169.254.169.254 --linklocal_service_port 80 --ipfabric_service_ip \"${openstack_ip_to_use}\"  --ipfabric_service_port 8775 --oper add && echo provision-metadata-services >> /etc/contrail/contrail_config_exec.out",
