class contrail::provision_role_config (
    ...
    exec { 'provision-role-config' :
            command   => "python /opt/contrail/provision_role.py ${config_ip_list_for_shell} ${config_name_list_for_shell} ${api_server_ip} ${keystone_admin_user} ${keystone_admin_password} ${keystone_admin_tenant} 'config' && echo provision-role-config >> /etc/contrail/contrail_config_exec.out",
            provider  => shell,
